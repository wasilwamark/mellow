...
func (p *Plugin) installNode(ctx context.Context, conn plugin.Connection, version string, pass string) error {
	fmt.Printf("📦 Installing Node.js %s...\n", version)

	// Check if nvm exists
	if result := conn.RunCommand("command -v nvm", plugin.WithHideOutput()); !result.Success {
		...
		if result := conn.RunCommand(installCmd, plugin.WithHideOutput()); !result.Success {
		...
		if result := conn.RunCommand(profileCmd, plugin.WithHideOutput()); !result.Success {
	...
	if result := conn.RunCommand(verifyCmd, plugin.WithHideOutput()); !result.Success {
...
func (p *Plugin) installPython(ctx context.Context, conn plugin.Connection, version string, pass string) error {
	...
	if result := conn.RunCommand("command -v uv", plugin.WithHideOutput()); !result.Success {
		...
		conn.RunCommand(exportCmd, plugin.WithHideOutput())
	}
	...
	if result := conn.RunCommand(pinCmd, plugin.WithHideOutput()); !result.Success {
...
func (p *Plugin) installGo(ctx context.Context, conn plugin.Connection, version string, pass string) error {
	...
	if result := conn.RunCommand(downloadCmd, plugin.WithHideOutput()); !result.Success {
	...
	conn.RunCommand(cleanupCmd, plugin.WithHideOutput())
...
func (p *Plugin) installJava(ctx context.Context, conn plugin.Connection, version string, pass string) error {
	...
	if result := conn.RunCommand(homeCmd, plugin.WithHideOutput()); !result.Success {
...
func (p *Plugin) installRust(ctx context.Context, conn plugin.Connection, version string, pass string) error {
...
func (p *Plugin) installPHP(ctx context.Context, conn plugin.Connection, version string, pass string) error {
...
func (p *Plugin) installRuby(ctx context.Context, conn plugin.Connection, version string, pass string) error {
	...
	if result := conn.RunCommand("test -d ~/.rbenv", plugin.WithHideOutput()); !result.Success {
		...
		if result := conn.RunCommand(rbenvCmd, plugin.WithHideOutput()); !result.Success {
...
func (p *Plugin) installDotNet(ctx context.Context, conn plugin.Connection, version string, pass string) error {
	fmt.Printf("📦 Installing .NET %s...\n", version)

	// Detect Ubuntu version
	ubuntuVersion := "20.04"
	if result := conn.RunCommand(`lsb_release -rs | cut -d. -f1`, plugin.WithHideOutput()); result.Success {
...
func (p *Plugin) listHandler(ctx context.Context, conn plugin.Connection, args []string, flags map[string]interface{}) error {
	...
	for _, rt := range runtimes {
		fmt.Printf("=== %s ===\n", rt.name)
		if result := conn.RunCommand(rt.cmd, plugin.WithHideOutput()); result.Success {
		...
		if rt.extra != "" {
			if result := conn.RunCommand(rt.extra, plugin.WithHideOutput()); result.Success && strings.TrimSpace(result.Stdout) != "" {
...
func (p *Plugin) useHandler(ctx context.Context, conn plugin.Connection, args []string, flags map[string]interface{}) error {
	if len(args) < 2 {
...
func (p *Plugin) useNode(ctx context.Context, conn plugin.Connection, version string) error {
	fmt.Printf("🔄 Switching to Node.js %s...\n", version)

	cmd := fmt.Sprintf(`bash -c 'source ~/.nvm/nvm.sh && nvm use %s && nvm alias default %s'`, version, version)
	if result := conn.RunCommand(cmd, plugin.WithHideOutput()); !result.Success {
...
func (p *Plugin) usePython(ctx context.Context, conn plugin.Connection, version string) error {
...
func (p *Plugin) removeHandler(ctx context.Context, conn plugin.Connection, args []string, flags map[string]interface{}) error {
	if len(args) < 2 {
	...
	case "java", "jdk":
		...
		result := conn.RunCommand(findCmd, plugin.WithHideOutput())
	...
	case "rust":
		...
		if result := conn.RunCommand(cmd, plugin.WithHideOutput()); !result.Success {
			...
			if result := conn.RunCommand(removeCmd, plugin.WithHideOutput()); !result.Success {
...
func (p *Plugin) statusHandler(ctx context.Context, conn plugin.Connection, args []string, flags map[string]interface{}) error {
	...
	for _, c := range commands {
		fmt.Printf("=== %s ===\n", c.name)
		if result := conn.RunCommand(c.cmd, plugin.WithHideOutput()); result.Success && strings.TrimSpace(result.Stdout) != "" {
			for _, line := range strings.Split(strings.TrimSpace(result.Stdout), "\n") {
				fmt.Printf("  %s\n", line)
			}
		} else {
...
func (p *Plugin) updateHandler(ctx context.Context, conn plugin.Connection, args []string, flags map[string]interface{}) error {
	fmt.Println("🔄 Updating runtime version managers...")

	// Update nvm
	fmt.Println("\n📦 Updating NVM...")
	if result := conn.RunCommand(`bash -c 'source ~/.nvm/nvm.sh && nvm update-version'`, plugin.WithHideOutput()); !result.Success {
		fmt.Println("  NVM update failed or not installed")
	}

	// Update uv
	fmt.Println("\n📦 Updating uv...")
	if result := conn.RunCommand(`bash -c 'export PATH="$HOME/.local/bin:$PATH" && uv self update'`, plugin.WithHideOutput()); !result.Success {
		fmt.Println("  uv update failed or not installed")
	}

	// Update rbenv
	fmt.Println("\n📦 Updating rbenv...")
	if result := conn.RunCommand(`bash -c 'export PATH="$HOME/.rbenv/bin:$PATH" && eval "$(rbenv init -)" && rbenv update'`, plugin.WithHideOutput()); !result.Success {
		fmt.Println("  rbenv update failed or not installed")
	}

	// Update rustup
	fmt.Println("\n📦 Updating Rust...")
	if result := conn.RunCommand(`bash -c 'rustup update'`, plugin.WithHideOutput()); !result.Success {
