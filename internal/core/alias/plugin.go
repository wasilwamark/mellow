...
// Command handlers
func (p *Plugin) handleAdd(ctx context.Context, conn plugin.Connection, args []string, flags map[string]interface{}) error {
	if len(args) < 2 {
...
func (p *Plugin) handleRemove(ctx context.Context, conn plugin.Connection, args []string, flags map[string]interface{}) error {
	if len(args) < 1 {
